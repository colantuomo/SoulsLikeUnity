...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private Vector2 _currentRollInputDirection;

        // Only reason to create this localInput is to do a better roll
        private Vector2 _localInputMovement;
        ...
        private void Move()
        {
            if (_input.move == Vector2.zero) PlayerStates.currentState = States.Idle;
            else PlayerStates.currentState = States.Walking;
            // set target speed based on move speed, sprint speed and if sprint is pressed
            float targetSpeed = _input.sprint ? SprintSpeed : MoveSpeed;
            _localInputMovement = _input.move;
            ...
            if (_isRolling)
            {
                _localInputMovement = _currentRollInputDirection;
            }
            else
            {
                _localInputMovement = _input.move;
            }
            if (_localInputMovement == Vector2.zero) targetSpeed = 0.0f;

            // a reference to the players current horizontal velocity
            float currentHorizontalSpeed = new Vector3(_controller.velocity.x, 0.0f, _controller.velocity.z).magnitude;

            float speedOffset = 0.1f;
            float inputMagnitude = _input.analogMovement ? _localInputMovement.magnitude : 1f;
            ...
            Vector3 inputDirection = new Vector3(_localInputMovement.x, 0.0f, _localInputMovement.y).normalized;

            // note: Vector2's != operator uses approximation so is not floating point error prone, and is cheaper than magnitude
            // if there is a move input rotate player when the player is moving
            if (_localInputMovement != Vector2.zero)
            {
        ...
        {
            _isRolling = false;
            _localInputMovement = Vector2.zero;
        }
